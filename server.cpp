...
inline std::vector<std::string> parse_command_line(const std::string& line) {
    ...
    if (required_quote != 0) {
        ...
        if (command_upper == "JSON.UPDATE") { for(size_t i=1; i<args.size(); ++i) { std::string lower_arg=args[i]; std::transform(lower_arg.begin(),lower_arg.end(),lower_arg.begin(),[](unsigned char c){return ::tolower(c);}); if(lower_arg=="where")args[i]="WHERE"; else if(lower_arg=="set")args[i]="SET"; } }
    }
    return args;
}
